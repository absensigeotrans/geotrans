-- Migration: Split driver role into driver_bebas and driver_kantor
-- Created: 2026-05-26
-- Purpose:
--   1. Add new driver role types to user_role_enum
--   2. Migrate existing driver profiles to driver_bebas
--   3. Update validate_attendance_geofence trigger:
--      - driver_bebas: bypass geofence (bisa absen di mana saja), tetap kena late check & overtime
--      - driver_kantor: geofence enforced (harus absen di kantor)

-- ============================================================
-- 1. UPDATE ENUM
-- ============================================================

ALTER TYPE user_role_enum ADD VALUE IF NOT EXISTS 'driver_bebas';
ALTER TYPE user_role_enum ADD VALUE IF NOT EXISTS 'driver_kantor';
...
SET role = 'driver_bebas'
WHERE role = 'driver';

-- Also update auth.users raw_user_meta_data for consistency
UPDATE auth.users
SET raw_user_meta_data = raw_user_meta_data || '{"role": "driver_bebas"}'::jsonb
