-- Migration: 1020_fix_geofence_office_selection.sql
-- Fix: Restore NEW.office_id check + add ORDER BY to prevent
-- non-deterministic office selection when multiple offices exist.
--
-- Problem:
--   Server trigger used `WHERE is_active = true LIMIT 1` (no ORDER BY),
--   client used `.limit(1)` without filter/order. With >1 active offices
--   each query could pick a different office, causing geofence validation
--   to fail even when user was at the correct location.
--
-- Fix:
--   1. Restore original logic: use NEW.office_id if provided
--   2. Fallback: WHERE is_active = true ORDER BY created_at DESC LIMIT 1
